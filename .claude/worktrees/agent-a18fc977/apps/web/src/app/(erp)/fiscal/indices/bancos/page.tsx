...
export default function BancosPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/ref-tables/bancos?${params}`);
      if (res.ok) {
  ...
  const TIPO_LABELS: Record<string, string> = {
    ...
    BANCO_COOPERATIVO: 'Banco Cooperativo',
    INSTITUICAO_PAGAMENTO: 'Inst. Pagamento',
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">Cadastro de instituições financeiras do SFN</p>
        </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50">
              ...
              <th className="px-4 py-3 text-left font-medium text-slate-600 w-24">ISPB</th>
              <th className="px-4 py-3 text-left font-medium text-slate-600">Nome</th>
              <th className="px-4 py-3 text-left font-medium text-slate-600 w-28">Abreviação</th>
              <th className="px-4 py-3 text-left font-medium text-slate-600 w-40">Tipo</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              ...
              <tr key={item.id} className="hover:bg-slate-50">
                ...
                <td className="px-4 py-3 font-mono text-slate-500 text-xs">{item.ispb || '-'}</td>
                ...
                <td className="px-4 py-3">
                  {item.tipo ? (
                    <span className="inline-flex px-2 py-0.5 rounded text-xs font-medium bg-cyan-50 text-cyan-700">
