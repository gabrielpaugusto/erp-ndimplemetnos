...
export default function CstPisCofinsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/ref-tables/cst-pis-cofins?${params}`);
      if (res.ok) {
  ...
  }, [page, search, tipo, regime]);

  useEffect(() => { load(); }, [load]);
  useEffect(() => { setPage(1); }, [search, tipo, regime]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">CST PIS/COFINS</h1>
          <p className="text-slate-500 text-sm mt-0.5">Código de Situação Tributária do PIS e da COFINS</p>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-4">
        <div className="flex flex-wrap gap-3">
          ...
          >
            ...
            <option value="AMBOS">Ambos regimes</option>
          </select>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50">
              ...
              <th className="px-4 py-3 text-left font-medium text-slate-600 w-40">Regime</th>
            </tr>
          </thead>
              ...
              <tr key={item.id} className="hover:bg-slate-50">
                ...
                <td className="px-4 py-3">
                  <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${
                    item.tipo === 'ENTRADA' ? 'bg-blue-50 text-blue-700' :
                    item.tipo === 'SAIDA' ? 'bg-green-50 text-green-700' :
                ...
                <td className="px-4 py-3">
                  <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${
                    item.regime === 'NAO_CUMULATIVO' ? 'bg-purple-50 text-purple-700' : 'bg-gray-100 text-gray-600'
                  }`}>
