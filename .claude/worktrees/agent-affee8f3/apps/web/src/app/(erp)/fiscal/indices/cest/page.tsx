...
export default function CestPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/ref-tables/cest?${params}`);
      if (res.ok) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">CEST</h1>
          <p className="text-slate-500 text-sm mt-0.5">Código Especificador da Substituição Tributária</p>
        </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50">
              <th className="px-4 py-3 text-left font-medium text-slate-600 w-32">Código</th>
              <th className="px-4 py-3 text-left font-medium text-slate-600 w-24">Segmento</th>
              <th className="px-4 py-3 text-left font-medium text-slate-600">Descrição</th>
              <th className="px-4 py-3 text-left font-medium text-slate-600 w-40">NCM Relacionado</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              ...
              <tr key={item.id} className="hover:bg-slate-50">
                <td className="px-4 py-3 font-mono text-slate-800">{item.codeFormatted || item.code}</td>
                <td className="px-4 py-3">
                  <span className="inline-flex px-2 py-0.5 rounded text-xs font-medium bg-indigo-50 text-indigo-700">
                    {item.segmento}
                  </span>
                </td>
                <td className="px-4 py-3 text-slate-600 text-xs leading-tight">{item.description}</td>
                <td className="px-4 py-3 text-slate-500 text-xs">{item.ncmRelacionado || '-'}</td>
              </tr>
            ))}
          </tbody>
        </table>
