...
export class TaxEngineService {
  ...
  /**
   * Find the best matching TaxRule (highest priority) for NCM+CFOP+operation.
   */
  private async findBestRule(
    ...
    // Build progressive filters: most specific first
    ...
    // Also check partial NCM matches (e.g., rule "8704" matches item "87041000")
    if (rules.length === 0 && ncmCode.length > 4) {
      const partialNcm = ncmCode.substring(0, 4);
