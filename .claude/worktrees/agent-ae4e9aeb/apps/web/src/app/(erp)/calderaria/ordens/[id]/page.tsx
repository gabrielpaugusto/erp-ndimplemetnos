...
export default function CalderariaOrdemDetailPage() {
  ...
  const doAction = async (action: 'start' | 'complete' | 'cancel') => {
    ...
    try {
      ...
      alert(err instanceof Error ? err.message : 'Erro ao executar acao');
    } finally {
      ...
      <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="text-center">
            ...
            <p className={`text-2xl font-bold ${timeProgress > 100 ? 'text-red-600' : 'text-emerald-600'}`}>{timeProgress}%</p>
          </div>
        </div>
