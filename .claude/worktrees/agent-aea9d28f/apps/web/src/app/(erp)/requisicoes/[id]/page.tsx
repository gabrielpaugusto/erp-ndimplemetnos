...
export default function RequisicaoDetailPage() {
    ...
    try {
      ...
      setRequisition({
        ...
        timeline: {
          ...
          solicitada: ['SOLICITADA', 'APROVADA', 'SEPARADA', 'ENTREGUE'].includes(data.status) ? data.createdAt : null,
  ...
  const statusSteps: ReqStatus[] = ['RASCUNHO', 'SOLICITADA', 'APROVADA', 'SEPARADA', 'ENTREGUE'];
  const statusOrder: Record<ReqStatus, number> = {
    RASCUNHO: 0, SOLICITADA: 1, APROVADA: 2, SEPARADA: 3, ENTREGUE: 4, CANCELADA: -1,
  };
