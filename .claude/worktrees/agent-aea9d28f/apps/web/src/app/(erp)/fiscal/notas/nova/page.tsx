...
export default function NovaNFeFormPage() {
  ...
  const handleSave = async () => {
    if (!form.pessoaId) { alert('Selecione o destinatário/remetente.'); return; }
    if (!form.naturezaOperacao) { alert('Informe a natureza da operação.'); return; }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        alert(err.message || 'Erro ao criar NF-e');
        return;
      }

      router.push('/fiscal/notas');
    } catch {
      alert('Erro ao criar NF-e');
    } finally {
      setSaving(false);
    }
  };
            ...
            <div key={item.id} className="border border-slate-200 rounded-lg overflow-hidden">
              ...
              {item.showTaxes && (
                <div className="bg-slate-50 border-t border-slate-200 p-4">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-teal-700 mb-2">Reforma Tributária (IBS + CBS + IS)</p>
