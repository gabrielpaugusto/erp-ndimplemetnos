...
export class StockMovementsService {
  ...
  async create(companyId: string, userId: string, data: CreateStockMovementDto) {
    ...
    // Mapeamento de source para flag de finalidade no produto
    const sourceToFlag: Record<string, string> = {
    ...
    const flagField = sourceToFlag[data.source];
    if (flagField && (data.type === 'SAIDA' || data.type === 'CONSUMO_INTERNO')) {
      ...
      if (!(product as any)[flagField]) {
        ...
        throw new BadRequestException(
          `Este item não está habilitado para saída por ${flagLabels[flagField]}. Verifique as flags de finalidade no cadastro do produto.`
        );
      }

      // Se não controla estoque: saída é automática, não atualiza saldo de estoque
      if (!(product as any).controlaEstoque) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      // Atualiza custo médio e custo último quando ENTRADA via COMPRA
      if (data.type === 'ENTRADA' && data.source === 'COMPRA' && data.unitCost && data.unitCost > 0) {
        const currentBalance = await tx.stockBalance.findFirst({
          where: { productId: data.productId, locationId: data.locationId },
        });

        const saldoAtual = Number(currentBalance?.quantity || 0);
        const custoMedioAtual = Number(currentBalance?.averageCost || 0);
        const novoCustoMedio = saldoAtual > 0
          ? (saldoAtual * custoMedioAtual + data.quantity * data.unitCost) / (saldoAtual + data.quantity)
          : data.unitCost;

        await tx.product.update({
          where: { id: data.productId },
          data: {
            custoUltimo: data.unitCost,
