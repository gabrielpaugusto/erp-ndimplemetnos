...
export class PurchaseRequestsService {
  ...
  async approve(id: string, userId: string, companyId: string) {
    ...
    // Calculate estimated value for threshold-based approval policies
    const estimatedValue = request.items.reduce(
      (sum: number, item: any) => sum + Number(item.quantity) * Number(item.estimatedPrice ?? 0),
      0,
    );
    ...
    // No approval policy → advance directly to APROVADA
    return this.prisma.purchaseRequest.update({
      where: { id },
      data: {
