...
export class CteController {
  ...
  /** Status da última sincronização CT-e (rate limit, última chamada, etc.) */
  @Get('sync-status')
  getSyncStatus(@CurrentUser() user: { companyId: string }) {
    return this.cteService.getSyncStatus(user.companyId);
  }

  /** Sincronizar CT-es recebidos via SEFAZ DistDFe */
  @Post('sync-sefaz')
  syncFromSefaz(@CurrentUser() user: { companyId: string }) {
    return this.cteService.syncFromSefaz(user.companyId);
  }
}
