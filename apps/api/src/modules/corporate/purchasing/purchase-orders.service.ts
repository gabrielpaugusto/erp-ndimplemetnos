...
import { CreatePurchaseOrderDto, DESTINACOES_COM_ESTOQUE } from './dto/create-purchase-order.dto';
...
export class PurchaseOrdersService {
  ...
  async findAll(
    ...
    const validStatuses = ['RASCUNHO', 'ENVIADA', 'CONFIRMADA', 'PARCIAL_RECEBIDA', 'RECEBIDA', 'CANCELADA'];
  ...
  async create(companyId: string, data: CreatePurchaseOrderDto) {
    ...
    return this.prisma.purchaseOrder.create({
      data: {
        ...
        finalidadeOperacao: (data.finalidadeOperacao as any) || 'INDUSTRIALIZACAO',
        ...
        items: {
          create: data.items.map((item) => ({
            ...
            icms: item.icms || 0,
            ipi: item.ipi || 0,
            destinacao: (item.destinacao as any) || 'MATERIA_PRIMA',
          })),
        },
      },
      include: {
  ...
  async update(id: string, data: UpdatePurchaseOrderDto) {
    ...
    if (data.finalidadeOperacao !== undefined) updateData.finalidadeOperacao = data.finalidadeOperacao;
  ...
  async send(id: string, userId?: string, companyId?: string) {
    const order = await this.prisma.purchaseOrder.findUnique({
      where: { id },
      include: { items: true },
    });
  ...
  async receive(id: string, userId: string, data: ReceiveItemsDto) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      for (const receiveItem of data.items) {
        ...
        // Create stock movement only for items that control inventory
        const destinacaoAtual = (existingItem as any).destinacao as string;
        const geraEstoque = DESTINACOES_COM_ESTOQUE.includes(destinacaoAtual as any);

        if (receiveItem.quantityReceived > 0 && geraEstoque) {
        ...
        // Para itens sem estoque (MATERIAL_USO_CONSUMO, IMOBILIZADO, SERVICO):
        // recebimento registrado (quantityReceived) mas sem StockMovement.
        // GGF gera StockMovement normalmente (entra em Almoxarifado GGF → CFOP 1101).
