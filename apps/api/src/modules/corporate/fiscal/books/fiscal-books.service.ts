...
// Mapeamento de código de obrigação → módulo fiscal responsável
// Usado para aplicar a data de início correta por módulo no calendário
const OBRIGACAO_MODULO: Record<string, FiscalModule> = {
  'DCTF':         FiscalModule.DCTFWEB,
  'DARF-PIS':     FiscalModule.EFD_CONTRIBUICOES,
  'DARF-COFINS':  FiscalModule.EFD_CONTRIBUICOES,
  'DARF-CSLL':    FiscalModule.DCTFWEB,
  'DARF-IRPJ':    FiscalModule.DCTFWEB,
  'EFD-ICMS':     FiscalModule.EFD_ICMS,
  'EFD-CONTRIB':  FiscalModule.EFD_CONTRIBUICOES,
  'FGTS':         FiscalModule.ESOCIAL,
  'GPS':          FiscalModule.ESOCIAL,
  'GIA':          FiscalModule.EFD_ICMS,
  'IRPJ-T':       FiscalModule.DCTFWEB,
  'CSLL-T':       FiscalModule.DCTFWEB,
  'ECF':          FiscalModule.SPED_ECF,
  'ECD':          FiscalModule.SPED_ECD,
  'RAIS':         FiscalModule.ESOCIAL,
  'DIRF':         FiscalModule.REINF,
};
...
export class FiscalBooksService {
  ...
  async getCalendarioObrigacoes(companyId: string, ano: number) {
    // Busca datas de início por módulo + fallback global
    ...
    const globalInicio: Date | null = company?.dataInicioOperacao ?? null;
    ...
    // Obrigação é "fora do escopo" se seu vencimento é anterior à data de início do módulo.
    // Prioridade: data do módulo específico → fallback global → sem filtro.
    const isAntesDaOperacao = (vencimento: Date, codigo: string): boolean => {
      const modulo = OBRIGACAO_MODULO[codigo];
      const dataInicio = modulo ? (byModule[modulo] ?? globalInicio) : globalInicio;
      if (!dataInicio) return false;
      return vencimento < dataInicio;
    };
    ...
    for (const ob of obrigacoes) {
      if (ob.frequencia === 'MENSAL') {
        for (let mes = 1; mes <= 12; mes++) {
          ...
          const antesDoInicio = isAntesDaOperacao(vencimento, ob.codigo);
          eventos.push({
            ...ob, mes, ano, vencimento: vencimento.toISOString().slice(0, 10),
            diasRestantes,
            antesDoInicio,
            atrasado: !antesDoInicio && diasRestantes < 0,
            urgente: !antesDoInicio && diasRestantes >= 0 && diasRestantes <= 10,
          });
        }
