...
// Fallback quando não há CompanyTaxRate cadastrado para a empresa.
// IBS/CBS são 0.0 por padrão — a reforma tributária não está ativa em todas as empresas.
// Use CompanyTaxRate.aliquotaIbs / .aliquotaCbs para ativar por empresa.
const DEFAULT_ALIQ = {
  pisLR: 1.65,   // Lucro Real — não cumulativo (Lei 10.637/2002 e 10.833/2003)
  cofinsLR: 7.6,
  pisLP: 0.65,   // Lucro Presumido — cumulativo (Lei 9.718/1998)
  cofinsLP: 3.0,
  ibs: 0.0,      // IBS — reforma tributária inativa por padrão (configurar via CompanyTaxRate)
  cbs: 0.0,      // CBS — reforma tributária inativa por padrão (configurar via CompanyTaxRate)
};
...
export class FiscalEngineAutomationService {
  ...
  async resolveItem(
    ...
    const company = await this.prisma.company.findUnique({
      ...
      select: {
        ...
        uf: true,
        mvaPadrao: true,
        cfopPadraoNfeSaida: true,
        cfopPadraoNfeSaidaInter: true,
        cstIcmsPadrao: true,
        csosnPadrao: true,
      },
    });
    ...
    // Buscar alíquotas configuradas na tabela CompanyTaxRate (vigência atual)
    const now = new Date();
    const companyTaxRate = await (this.prisma as any).companyTaxRate.findFirst({
      where: {
    ...
    // Fallback configurável por empresa (CFOP padrão de venda intra/interestadual)
    const cfopFallbackIntra = (company as any)?.cfopPadraoNfeSaida ?? '5102';
    const cfopFallbackInter = (company as any)?.cfopPadraoNfeSaidaInter ?? '6102';
    if (!cfopEntry) {
      avisos.push(`Tipo de operação '${context.operationType}' não mapeado — usando CFOP padrão da empresa`);
    }
    ...
    if (context.exterior) {
      ...
      cfopCode = cfopEntry?.intra ?? cfopFallbackIntra;
    } else {
      cfopCode = cfopEntry?.inter ?? cfopFallbackInter;
    }
    ...
    // 4. TaxRule removed — FiscalBrain handles fiscal classification
    const taxRule: any = await this.findBestRule(
      context.companyId,
      item.ncmCode ?? '',
      cfopCode,
    ...
    // CST/CSOSN padrão: config da empresa > constante do sistema
    // (TaxRule removed; NCM fields cstIcms/csosn removed — FiscalBrain handles classification)
    const cstPadraoEmpresa  = (company as any)?.cstIcmsPadrao ?? CST_ICMS_SAIDA_PADRAO;
    const csosnPadraoEmpresa = (company as any)?.csosnPadrao  ?? CSOSN_SAIDA_PADRAO;

    if (isSimplesNacional) {
      csosn = taxRule?.cstIcms
        ?? (isSaida ? csosnPadraoEmpresa : CSOSN_ENTRADA_PADRAO);
    } else {
      cstIcms = taxRule?.cstIcms
        ?? (isSaida ? cstPadraoEmpresa : CST_ICMS_ENTRADA_PADRAO);
    }
    ...
    const reducaoBcIcms = Number(taxRule?.reducaoBcIcms ?? 0);
    const bcIcms = this.round(totalPrice * (1 - reducaoBcIcms / 100));
    const valorIcms = isSimplesNacional ? 0 : this.round(bcIcms * aliqIcms / 100);

    // 7. ICMS-ST
    const temSt = (ncm as any)?.temSt ?? false;
    // ST simplificado (base: MVA configurável na empresa — regras específicas por UF não implementadas aqui)
    const mva = ((company as any)?.mvaPadrao ?? 30.0) / 100;
    const aliqIcmsSt = temSt && !isSimplesNacional ? aliqIcms : 0;
    const bcIcmsSt = temSt && !isSimplesNacional ? this.round(totalPrice * (1 + mva)) : 0;
    ...
    // 8. IPI (NCM fields removed — FiscalBrain handles; use system defaults)
    const cstIpi = taxRule?.cstIpi ?? (isSaida ? '50' : '00');
    const aliqIpi = Number(taxRule?.aliqIpi ?? 0);
    const bcIpi = totalPrice;
    const valorIpi = this.round(bcIpi * aliqIpi / 100);

    // 9. PIS / COFINS (NCM fields removed — FiscalBrain handles; use system defaults)
    const cstPis = taxRule?.cstPis
      ?? (isSaida ? this.defaultCstPisCofins(regime, isSaida) : '98');
    ...
    if (isSimplesNacional) {
      ...
      const defaultPisLR = companyTaxRate?.aliquotaPis ?? DEFAULT_ALIQ.pisLR;
      const defaultCofinsLR = companyTaxRate?.aliquotaCofins ?? DEFAULT_ALIQ.cofinsLR;
      aliqPis = Number(taxRule?.aliqPis ?? defaultPisLR);
      aliqCofins = Number(taxRule?.aliqCofins ?? defaultCofinsLR);
    } else {
      // Lucro Presumido — cumulativo
      const defaultPisLP = companyTaxRate?.aliquotaPis ?? DEFAULT_ALIQ.pisLP;
      const defaultCofinsLP = companyTaxRate?.aliquotaCofins ?? DEFAULT_ALIQ.cofinsLP;
      aliqPis = Number(taxRule?.aliqPis ?? defaultPisLP);
      aliqCofins = Number(taxRule?.aliqCofins ?? defaultCofinsLP);
    }
    ...
    // Reforma Tributária (NCM fields removed — FiscalBrain handles; use company rates / system defaults)
    const aliqIbsBase = Number(taxRule?.aliqIbs ?? companyTaxRate?.aliquotaIbs ?? DEFAULT_ALIQ.ibs);
    const aliqCbsBase = Number(taxRule?.aliqCbs ?? companyTaxRate?.aliquotaCbs ?? DEFAULT_ALIQ.cbs);
    const aliqIsBase = Number(taxRule?.aliqIs ?? 0);
    ...
    const temIs = false; // NCM temIs field removed — FiscalBrain handles IS classification
    const aliqIs = 0;
    const valorIs = 0;
  ...
  /**
   * Resolve alíquota ICMS considerando origem e localização.
   * IcmsInterestadual table removed — FiscalBrain handles ICMS classification.
   */
  private async resolveAliqIcms(
    ...
    if (isInterestadual) {
      ...
      // IcmsInterestadual table removed — use standard interestadual rates
      // 7% for ES, RN, PB, PE, AL, SE, BA, PI, CE, MA, PA, AP, AM, RR, AC, RO, TO
      // 12% for all other states
      const nordesteSulNorte = ['ES', 'RN', 'PB', 'PE', 'AL', 'SE', 'BA', 'PI', 'CE', 'MA', 'PA', 'AP', 'AM', 'RR', 'AC', 'RO', 'TO'];
      const destinoIsNordesteNorte = nordesteSulNorte.includes(context.ufDestinatario?.toUpperCase());
      return destinoIsNordesteNorte ? 7 : 12;
    }

    // Intraestadual — 12% padrão
    return 12;
  }

  /** Mapeia OperationType → NFeOperation (enum do Prisma) */
  private toNfeOperation(operationType: string): string | null {
    const map: Record<string, string> = {
      SAIDA_VENDA_PRODUCAO:    'VENDA',
      SAIDA_VENDA_MERCADORIA:  'VENDA',
      SAIDA_VENDA_PECA:        'VENDA',
      SAIDA_SERVICO:           'VENDA',
      SAIDA_DEVOLUCAO_COMPRA:  'DEVOLUCAO_COMPRA',
      SAIDA_REMESSA_INDUSTRIA: 'REMESSA',
      SAIDA_REMESSA_CONSERTO:  'REMESSA',
      SAIDA_TRANSFERENCIA:     'TRANSFERENCIA',
      ENTRADA_COMPRA_INDUSTRIA:'COMPRA',
      ENTRADA_COMPRA_COMERCIO: 'COMPRA',
      ENTRADA_DEVOLUCAO_VENDA: 'DEVOLUCAO_VENDA',
      ENTRADA_RETORNO_INDUSTRIA:'RETORNO',
      ENTRADA_RETORNO_CONSERTO: 'RETORNO',
   ...
   * TaxRule model removed — FiscalBrain AI engine handles fiscal classification automatically.
  ...
  private async findBestRule(
    ...
    // TaxRule table removed — FiscalBrain handles classification
    return null;
  }
