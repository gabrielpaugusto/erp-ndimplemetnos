...
export class NfeXmlBuilderService {
  ...
  async buildXml(nfeId: string, ambiente: '1' | '2'): Promise<{ xml: string; chaveAcesso: string; cNf: string }> {
    ...
    let vBCIBS = 0, vIBS = 0, vBCCBS = 0, vCBS = 0, vBCIS = 0, vIS = 0;
    for (const item of nfe.items) {
      ...
      vBCIBS += Number((item as any).bcIbs ?? 0);
      vIBS   += Number((item as any).valorIbs ?? 0);
      vBCCBS += Number((item as any).bcCbs ?? 0);
      vCBS   += Number((item as any).valorCbs ?? 0);
      vBCIS  += Number((item as any).bcIs ?? 0);
      vIS    += Number((item as any).valorIs ?? 0);
    }
    ...
    const destAddrUf = (dest as any)?.addresses?.[0]?.uf;
    const idDest = !destAddrUf ? '1'
      : destAddrUf === 'EX' ? '3'
      : destAddrUf.toUpperCase() === (company.uf ?? 'SP').toUpperCase() ? '1'
      : '2';

    // indIEDest: 1=contribuinte ICMS, 2=contribuinte isento, 9=não contribuinte
    const indIEDest = dest?.rgIe ? '1' : '9';

    // Endereço do destinatário: usa o endereço principal ou o primeiro cadastrado
    const destAddr = (dest as any)?.addresses?.[0];
    ...
    const itemsXml = nfe.items.map((item, i) => {
      ...
      const isTag = isValor > 0
        ? `<IS>
              <CST>01</CST>
              <vBCIS>${this.fmt(isBc)}</vBCIS>
              <pAliqIS>${this.fmt(isAliq, 4)}</pAliqIS>
              <vIS>${this.fmt(isValor)}</vIS>
            </IS>`
        : '';
...
<NFe xmlns="http://www.portalfiscal.inf.br/nfe">
  <infNFe versao="4.00" Id="NFe${chaveAcesso}">
    ...
    <dest>
      ...
      <enderDest>
        <xLgr>${this.sanitize(destAddr?.logradouro ?? 'Rua Nao Informada', 60)}</xLgr>
        <nro>${this.sanitize(destAddr?.numero ?? 'SN', 10)}</nro>
        ${destAddr?.complemento ? `<xCpl>${this.sanitize(destAddr.complemento, 60)}</xCpl>` : ''}
        <xBairro>${this.sanitize(destAddr?.bairro ?? 'Nao Informado', 60)}</xBairro>
        <cMun>${destAddr?.codigoMunicipioIbge ?? '9999999'}</cMun>
        <xMun>${this.sanitize(destAddr?.municipio ?? 'Nao Informado', 60)}</xMun>
        <UF>${destAddr?.uf ?? 'SP'}</UF>
        <CEP>${(destAddr?.cep ?? '').replace(/\D/g, '').padStart(8, '0')}</CEP>
        <cPais>1058</cPais>
        <xPais>Brasil</xPais>
      </enderDest>
    ...
    <total>
      ...
      ${(vIBS > 0 || vCBS > 0) ? `<totIBSCBS>
        <vBCIBS>${this.fmt(vBCIBS)}</vBCIBS>
        <vIBS>${this.fmt(vIBS)}</vIBS>
        <vBCCBS>${this.fmt(vBCCBS)}</vBCCBS>
        <vCBS>${this.fmt(vCBS)}</vCBS>
        <vBCIS>${this.fmt(vBCIS)}</vBCIS>
        <vIS>${this.fmt(vIS)}</vIS>
      </totIBSCBS>` : ''}
    </total>
    <transp>
      <modFrete>9</modFrete>
