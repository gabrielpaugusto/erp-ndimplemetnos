...
export class FinancialMovementsService {
  ...
  async pay(id: string, data: PayMovementDto, userId?: string, companyId?: string) {
    const movement = await this.prisma.financialMovement.findUnique({
      where: { id },
    });
    ...
    // Check if approval is needed (only for DESPESA type)
    if (movement.type === 'DESPESA' && userId && companyId) {
      ...
      if (needsApproval && movement.status !== 'AGUARDANDO_APROVACAO') {
