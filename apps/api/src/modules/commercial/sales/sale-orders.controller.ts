...
export class SaleOrdersController {
  ...
  approve(
    ...
    return this.saleOrdersService.approve(id, user.id, user.companyId);
  }
  ...
  getTimeline(
    ...
    return this.saleOrdersService.getTimeline(id, user.companyId);
  }
  ...
  getReservas(
    ...
    return this.saleOrdersService.getReservas(id, user.companyId);
  }

  // ── Sprint 3.4 — Customização do Pedido ─────────────────────────────────
  ...
  getCustomizacao(
    ...
    return this.saleOrdersService.getCustomizacao(id, user.companyId);
  }

  @Post(':id/customizacao')
  upsertCustomizacao(
    @CurrentUser() user: { id: string; companyId: string },
    @Param('id') id: string,
    @Body() body: any,
  ) {
    return this.saleOrdersService.upsertCustomizacao(id, user.companyId, body);
  }
}
