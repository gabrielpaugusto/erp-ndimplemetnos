...
export class SaleOrdersService {
  ...
  async approve(id: string, userId: string, companyId: string) {
    ...
    if (needsApproval) {
      ...
      return this.prisma.saleOrder.update({
        where: { id },
        data: { status: 'PENDENTE_APROVACAO' as any },
        include: {
          person: true,
          items: { include: { product: true } },
        },
  ...
  async getTimeline(id: string, companyId: string) {
    return this.documentEvents.getTimeline('SaleOrder', id, companyId);
  }

  // ── Sprint 2.4 — Reservas de Estoque ───────────────────────────────────

  async getReservas(id: string, companyId: string) {
    return this.stockReservations.listBySource(companyId, 'SALE_ORDER', id);
  }

  // ── Sprint 3.4 — Customização ──────────────────────────────────────────

  async getCustomizacao(id: string, companyId: string) {
    return this.customization.getCustomization(id, companyId);
  }

  async upsertCustomizacao(id: string, companyId: string, dto: any) {
    return this.customization.upsertCustomization(id, companyId, dto);
  }
}
