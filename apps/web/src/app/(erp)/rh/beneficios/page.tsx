...
export default function BeneficiosPage() {
  ...
  const handleAdd = async () => {
    ...
    try {
      const res = await apiFetch('/api/hr/benefits', {
        method: 'POST',
        body: JSON.stringify({
          employeeId: newFuncionario,
          type: newTipo,
          valorEmpresa: parseFloat(newValor),
        }),
      });
      if (!res.ok) {
        const d = await res.json().catch(() => ({}));
        setAddError(d.message || 'Erro ao adicionar beneficio');
        return;
      }
      ...
      setAddError('Erro de conexao. Verifique se o servidor esta rodando.');
    } finally {
      setAddSaving(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      {showAddForm && (
        <div className="bg-white rounded-lg shadow-sm border border-sky-200 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select value={newTipo} onChange={(e) => setNewTipo(e.target.value)} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent">
                <option value="">Selecione...</option>
                {Object.entries(typeToLabel).map(([key, label]) => <option key={key} value={key}>{label}</option>)}
              </select>
            </div>
            <div>
          ...
          {addError && (
            <div className="mt-3 bg-red-50 border border-red-200 rounded-lg px-4 py-3 text-sm text-red-700">{addError}</div>
          )}
          <div className="flex items-center justify-end gap-3 mt-4">
            <button onClick={() => { setShowAddForm(false); setAddError(''); }} className="inline-flex items-center gap-2 px-4 py-2 text-sm text-slate-600 hover:text-slate-800">
              <X className="w-4 h-4" />Cancelar
            </button>
            <button onClick={handleAdd} disabled={addSaving || !newFuncionario || !newTipo || !newValor} className="inline-flex items-center gap-2 px-4 py-2 bg-sky-600 text-white rounded-lg hover:bg-sky-700 text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
              <Save className="w-4 h-4" />{addSaving ? 'Salvando...' : 'Salvar'}
            </button>
          </div>
        </div>
      )}
