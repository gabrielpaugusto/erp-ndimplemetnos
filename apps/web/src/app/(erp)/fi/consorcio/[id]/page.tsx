...
export default function ConsorcioDetailPage() {
  ...
  const handleRegisterPayment = async (paymentId: string) => {
    ...
    try {
      const res = await apiFetch(`/api/fi/consortium/payments/${paymentId}`, {
        method: 'PATCH',
      ...
      setConsortium((prev) =>
        prev
          ? {
              ...prev,
              parcelasPagas: (prev.parcelasPagas ?? 0) + 1,
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-4">
          ...
          <p className="text-sm font-semibold text-slate-900">
            {consortium.comissaoPercent != null ? fmtPercent(consortium.comissaoPercent) : '—'}
          </p>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="text-sm font-semibold text-slate-700 whitespace-nowrap">
            {parcelasPagas} / {parcelasMensais} ({fmtPercent(progressPct, 1)})
          </span>
        </div>
      </div>
                  ...
                  <button
                    onClick={async () => {
                      try {
                        ...
                        toast.error(err.message || 'Erro ao registrar contemplação');
                      }
                    }}
                    className="inline-flex items-center gap-2 px-4 py-2 bg-violet-600 text-white rounded-lg hover:bg-violet-700 text-sm font-medium transition-colors"
                  >
                    ...
                    <td className="px-4 py-3 text-right">
                      {(payment.status === 'PENDENTE' || payment.status === 'VENCIDO') && (
                        <button
                          ...
                          className="inline-flex items-center gap-1 px-3 py-1 bg-emerald-600 text-white rounded text-xs font-medium hover:bg-emerald-700 transition-colors disabled:opacity-50"
                        >
                          <CheckCircle className="w-3 h-3" />
                          {payingId === payment.id ? 'Registrando...' : 'Registrar Pgto'}
                        </button>
                      )}
                      {payment.status === 'PAGO' && (
