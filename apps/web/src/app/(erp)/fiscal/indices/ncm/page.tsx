        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50 text-left">
              ...
              <th className="px-4 py-3 font-medium text-slate-600 text-center whitespace-nowrap">Ex TIPI</th>
              <th className="px-4 py-3 font-medium text-slate-600 text-center whitespace-nowrap">ST</th>
              <th className="px-4 py-3 font-medium text-slate-600 text-center whitespace-nowrap">CEST</th>
              <th className="px-4 py-3 font-medium text-slate-600 text-center whitespace-nowrap">Imp. %</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              [...Array(5)].map((_, i) => (
                <tr key={i}><td colSpan={8} className="px-4 py-3"><div className="h-4 bg-slate-200 rounded animate-pulse w-3/4" /></td></tr>
              ))
            ) : data.length === 0 ? (
              <tr><td colSpan={8} className="px-4 py-10 text-center text-slate-400">Nenhum NCM encontrado</td></tr>
            ) : data.map((item: any) => (
              <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-4 py-3 text-center text-slate-600 font-mono text-xs">{item.exTipi || '—'}</td>
                <td className="px-4 py-3 text-center">
                  {item.temSt
                    ? <span className="px-2 py-0.5 bg-orange-100 text-orange-700 rounded text-xs font-semibold">ST</span>
                    : <span className="text-slate-300">—</span>}
                </td>
                <td className="px-4 py-3 text-center font-mono text-xs text-slate-600">{item.cestCode || '—'}</td>
                <td className="px-4 py-3 text-center text-slate-600">
                  {item.aliquotaImportacao != null ? `${Number(item.aliquotaImportacao).toFixed(2)}%` : '—'}
                </td>
                <td className="px-4 py-3 text-center">
