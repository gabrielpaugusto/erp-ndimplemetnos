...
export default function NovaMovimentacaoPage() {
  ...
  const handleSave = async () => {
    ...
    if (!locationId) { toast.error('Selecione o local.'); return; }
    if (!quantity || parseFloat(quantity) <= 0) { toast.error('Informe a quantidade.'); return; }
    if (showDestination && !destLocationId) { toast.error('Selecione o local de destino.'); return; }
