...
import { ChevronLeft, Save, X, GitBranch, Lock } from 'lucide-react';
...
const sections = [
  ...
  { key: 'whereused', label: 'Onde é Utilizado' },
];
...
export default function EditarProdutoPage() {
  ...
  useEffect(() => {
    if (activeSection === 'whereused' && whereUsedData.length === 0 && !whereUsedLoading) {
      setWhereUsedLoading(true);
      apiFetch(`/api/products/${id}/where-used`)
        .then(r => r.json())
        .then(data => setWhereUsedData(Array.isArray(data) ? data : []))
        .catch(() => setWhereUsedData([]))
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-slate-200">
        ...
        <div className="p-6">
          ...
          {/* Onde é Utilizado (Where-Used) */}
          {activeSection === 'whereused' && (
            <div className="space-y-4">
              <div className="flex items-center gap-2 mb-2">
                <GitBranch className="w-5 h-5 text-blue-600" />
                <h3 className="text-base font-semibold text-slate-900">Onde Este Item é Utilizado</h3>
              </div>
              <p className="text-sm text-slate-500">
                Mostra todos os produtos pai que utilizam este item em suas listas de materiais (BOM).
              </p>

              {whereUsedLoading ? (
                <div className="text-sm text-slate-400 py-8 text-center">Buscando utilizações...</div>
              ) : whereUsedData.length === 0 ? (
                <div className="py-10 text-center">
                  <GitBranch className="w-10 h-10 text-slate-300 mx-auto mb-3" />
                  <p className="text-slate-500 text-sm font-medium">Este item não é componente de nenhum produto</p>
                  <p className="text-slate-400 text-xs mt-1">Não há BOMs que referenciem este item como componente.</p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full border-collapse">
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-200">
                        <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Produto Pai</th>
                        <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Grupo</th>
                        <th className="text-center px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Versão BOM</th>
                        ...
                        <th className="text-center px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">% Perda</th>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {whereUsedData.map((item) => (
                        <tr key={item.bomId} className="hover:bg-slate-50 transition-colors">
                          <td className="px-4 py-3">
                            <p className="text-sm font-medium text-slate-900">{item.productoPai.description}</p>
                            <p className="text-xs font-mono text-slate-400">{item.productoPai.code}</p>
                          </td>
                          <td className="px-4 py-3 text-sm text-slate-600">
                            {item.productoPai.group ? `${item.productoPai.group.code} — ${item.productoPai.group.name}` : '-'}
                          </td>
                          <td className="px-4 py-3 text-sm text-slate-700 text-center font-medium">v{item.bomVersion}</td>
                          <td className="px-4 py-3 text-sm font-semibold text-slate-900 text-center">{item.quantidade}</td>
                          <td className="px-4 py-3 text-sm text-slate-700 text-center">{item.unidade}</td>
                          <td className="px-4 py-3 text-sm text-slate-700 text-center">
                            {item.perdaPercent > 0 ? (
                              <span className="text-amber-600 font-medium">{item.perdaPercent}%</span>
                            ) : (
                          ...
                          <td className="px-4 py-3 text-center">
                            <Link
                              href={`/estoque/produtos/${item.productoPai.id}`}
                  ...
                  <p className="text-xs text-slate-400 mt-3 px-1">
                    Este item aparece em {whereUsedData.length} BOM(s).
                  </p>
                </div>
              )}
            </div>
