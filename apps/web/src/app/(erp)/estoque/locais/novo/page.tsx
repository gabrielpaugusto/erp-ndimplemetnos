...
export default function NovoLocalEstoquePage() {
  ...
  const handleSave = async () => {
    if (!code.trim() || !name.trim()) { setError('Codigo e Nome sao obrigatorios.'); return; }
    setSaving(true); setError('');
    try {
      const res = await apiFetch('/api/inventory/locations', {
        method: 'POST',
        body: JSON.stringify({ code: code.trim(), name: name.trim(), type, description: description.trim() || undefined }),
      });
      if (!res.ok) {
        const d = await res.json().catch(() => ({}));
        setError(d.message || 'Erro ao criar local de estoque');
        return;
      }
