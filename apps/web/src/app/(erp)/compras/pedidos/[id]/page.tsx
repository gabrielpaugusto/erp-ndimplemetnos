...
type OrderStatus = 'RASCUNHO' | 'AGUARDANDO_APROVACAO' | 'ENVIADA' | 'CONFIRMADA' | 'PARCIAL_RECEBIDA' | 'RECEBIDA' | 'CANCELADA';
...
type FinalidadeOperacao = 'INDUSTRIALIZACAO' | 'REVENDA' | 'USO_CONSUMO' | 'ATIVO_IMOBILIZADO';

const finalidadeLabels: Record<FinalidadeOperacao, string> = {
...
const finalidadeCfop: Record<FinalidadeOperacao, string> = {
  INDUSTRIALIZACAO:  'CFOP 1.101 / 2.101',
  REVENDA:           'CFOP 1.102 / 2.102',
  USO_CONSUMO:       'CFOP 1.120 / 2.120',
  ATIVO_IMOBILIZADO: 'CFOP 1.551 / 2.551',
};

const finalidadeColor: Record<FinalidadeOperacao, string> = {
  INDUSTRIALIZACAO:  'bg-blue-100 text-blue-700',
  REVENDA:           'bg-emerald-100 text-emerald-700',
  USO_CONSUMO:       'bg-slate-100 text-slate-600',
  ATIVO_IMOBILIZADO: 'bg-purple-100 text-purple-700',
};
...
interface OrderData {
  ...
  subtotal: number;
  total: number;
  finalidadeOperacao: FinalidadeOperacao;
  createdAt: string;
  timeline: {
...
type ItemDestinacao =
  ...
  | 'MATERIAL_USO_CONSUMO' | 'GGF' | 'IMOBILIZADO' | 'SERVICO';

const destinacaoLabels: Record<ItemDestinacao, string> = {
  MATERIA_PRIMA: 'Matéria-Prima',
  COMPONENTE: 'Componente',
  PRODUTO_REVENDA: 'Revenda',
  INSUMO_PRODUCAO: 'Insumo Prod.',
  EMBALAGEM: 'Embalagem',
  MATERIAL_USO_CONSUMO: 'Uso/Consumo',
  GGF: 'GGF',
  IMOBILIZADO: 'Imobilizado',
  SERVICO: 'Serviço',
};

const DESTINACOES_COM_ESTOQUE: ItemDestinacao[] = [
  'MATERIA_PRIMA', 'COMPONENTE', 'PRODUTO_REVENDA', 'INSUMO_PRODUCAO', 'EMBALAGEM',
];
...
function mapApiOrder(data: any): OrderData {
  return {
    ...
    finalidadeOperacao: (data.finalidadeOperacao ?? 'INDUSTRIALIZACAO') as FinalidadeOperacao,
...
function mapApiItems(data: any): OrderItem[] {
  return (data.items ?? []).map((item: any) => ({
    ...
    receivingQty: '',
    destinacao: (item.destinacao ?? 'MATERIA_PRIMA') as ItemDestinacao,
  }));
}
...
export default function PedidoCompraDetailPage() {
  ...
  const statusSteps: OrderStatus[] = ['RASCUNHO', 'AGUARDANDO_APROVACAO', 'ENVIADA', 'CONFIRMADA', 'PARCIAL_RECEBIDA', 'RECEBIDA'];
  const statusOrder: Record<OrderStatus, number> = {
    RASCUNHO: 0, AGUARDANDO_APROVACAO: 1, ENVIADA: 2, CONFIRMADA: 3, PARCIAL_RECEBIDA: 4, RECEBIDA: 5, CANCELADA: -1,
  };
        ...
        <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex justify-between items-center text-xs pt-1 border-t border-slate-100">
              ...
              <span className={`px-2 py-0.5 rounded-full font-medium ${finalidadeColor[order.finalidadeOperacao]}`}>
                {finalidadeLabels[order.finalidadeOperacao]}
              </span>
            </div>
            <div className="text-right">
              <span className="text-[10px] text-slate-400">{finalidadeCfop[order.finalidadeOperacao]}</span>
            </div>
          </div>
        </div>
      </div>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider w-28">Destinação</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {items.map((item) => {
                ...
                const temEstoque = DESTINACOES_COM_ESTOQUE.includes(item.destinacao);
                return (
                  <tr key={item.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        <span className={`shrink-0 w-2 h-2 rounded-full ${temEstoque ? 'bg-emerald-500' : 'bg-slate-300'}`} title={temEstoque ? 'Gera entrada de estoque' : 'Não gera estoque'} />
                        <span className="text-xs text-slate-600">{destinacaoLabels[item.destinacao] ?? item.destinacao}</span>
                      </div>
                    </td>
