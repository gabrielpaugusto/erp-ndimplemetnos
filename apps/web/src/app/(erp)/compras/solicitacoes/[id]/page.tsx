...
type RequestStatus = 'RASCUNHO' | 'SOLICITADA' | 'AGUARDANDO_APROVACAO' | 'APROVADA' | 'COTADA' | 'PEDIDA' | 'RECEBIDA' | 'CANCELADA';
...
export default function SolicitacaoCompraDetailPage() {
  ...
  const handleAction = async (action: 'approve' | 'cancel' | 'submit') => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : `Erro ao executar acao ${action}`);
    } finally {
  ...
  const statusSteps: RequestStatus[] = ['RASCUNHO', 'SOLICITADA', 'AGUARDANDO_APROVACAO', 'APROVADA', 'COTADA', 'PEDIDA', 'RECEBIDA'];
  const statusOrder: Record<RequestStatus, number> = {
    RASCUNHO: 0, SOLICITADA: 1, AGUARDANDO_APROVACAO: 2, APROVADA: 3, COTADA: 4, PEDIDA: 5, RECEBIDA: 6, CANCELADA: -1,
  };
