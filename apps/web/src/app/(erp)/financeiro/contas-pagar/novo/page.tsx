...
import { ChevronLeft, Save, X, ArrowDownRight, Layers } from 'lucide-react';
...
export default function NovoContaPagarPage() {
  ...
  useEffect(() => {
    apiFetch('/api/persons?limit=200').then(r => r.json()).then(d => setFornecedores(d.data ?? d ?? [])).catch(() => {});
  ...
  const categorias = ['4.1 - Materia-prima', '4.2 - Insumos de Producao', '4.3 - Despesas Operacionais', '4.4 - Despesas Administrativas', '4.5 - Servicos de Terceiros'];
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(d.message || 'Erro ao salvar titulo a pagar');
        return;
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-end gap-3">
        ...
        <button
          onClick={handleSave}
          disabled={saving || !descricao || !valor || !dataVencimento}
