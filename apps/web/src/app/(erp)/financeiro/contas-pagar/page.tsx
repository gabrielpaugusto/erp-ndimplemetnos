...
import {
  ...
  Calendar, ArrowDownRight, RefreshCw, CheckCircle2, Clock, AlertTriangle, Ban,
} from 'lucide-react';
import { apiFetch } from '@/lib/api';
import { fmtCurrency } from '@/lib/format';

type MovStatus = 'PENDENTE' | 'VENCIDO' | 'PAGO' | 'CANCELADO';
...
export default function ContasPagarPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const params = new URLSearchParams({ type: 'DESPESA', page: String(page), limit: String(limit) });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1">Gerencie os titulos a pagar</p>
        </div>
        <Link href="/financeiro/contas-pagar/novo" className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 text-sm font-medium transition-colors">
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white rounded-lg border border-emerald-200 p-4">
            <p className="text-xs text-slate-500">Pago (mes)</p>
            <p className="text-lg font-bold text-emerald-600">{fmt(stats.byStatus?.PAGO?.total ?? 0)}</p>
          </div>
          <div className="bg-white rounded-lg border border-slate-200 p-4">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-4">
        <div className="flex items-center gap-3">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Buscar por numero, fornecedor ou descricao..." value={search}
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                {['Numero', 'Fornecedor', 'Descricao', 'Valor', 'Vencimento', 'Status', 'Acoes'].map((h, i) => (
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-right">
                    <Link href={`/financeiro/contas-pagar/${item.id}`} className="p-1.5 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded transition-colors inline-flex">
