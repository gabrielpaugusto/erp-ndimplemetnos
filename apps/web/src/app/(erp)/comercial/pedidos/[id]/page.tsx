...
type OrderStatus = 'RASCUNHO' | 'PENDENTE_APROVACAO' | 'APROVADO' | 'FATURADO' | 'ENTREGUE' | 'CANCELADO';
...
interface SaleOrder {
  ...
  status: OrderStatus;
  saleType: string;
  finalidadeOperacao: FinalidadeOperacao | null;
  createdAt: string;
...
const STATUS_STEPS: OrderStatus[] = ['RASCUNHO', 'PENDENTE_APROVACAO', 'APROVADO', 'FATURADO', 'ENTREGUE'];
const STATUS_ORDER: Record<string, number> = {
  RASCUNHO: 0, PENDENTE_APROVACAO: 1, APROVADO: 2, FATURADO: 3, ENTREGUE: 4, CANCELADO: -1,
};
...
export default function PedidoDetailPage() {
  ...
  const canEmitirNfe  = (order.status === 'APROVADO' || order.status === 'RASCUNHO') && !nfeAutorizada;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {(order.status === 'RASCUNHO' || order.status === 'PENDENTE_APROVACAO' || order.status === 'APROVADO') && (
            <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          {order.vendedor ? (
            <>
              ...
              {order.comissaoPercent && (
                <>
                  <p className="text-xs text-slate-500 mt-1">Comissão: {fmtPercent(Number(order.comissaoPercent), 1)}</p>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-4">
          ...
          {order.finalidadeOperacao && (
            <p className="text-xs text-slate-500 mt-1">Finalidade: {finalidadeLabels[order.finalidadeOperacao] ?? order.finalidadeOperacao}</p>
          )}
        </div>
      </div>
