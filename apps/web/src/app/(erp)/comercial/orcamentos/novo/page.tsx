...
const formatCurrency = (v: number) => fmtCurrency(v);
...
export default function NovoOrcamentoPage() {
  ...
  const loadPersons = useCallback(async () => {
    try {
      const res = await apiFetch(`/api/crm/persons?role=CLIENTE&limit=100`);
      if (res.ok) {
  ...
  const loadProducts = useCallback(async () => {
    try {
      const res = await apiFetch(`/api/inventory/products?limit=200`);
      if (res.ok) {
        const data = await res.json();
  ...
  const handleSave = async (status: 'RASCUNHO' | 'ENVIADO') => {
    if (!form.personId) { toast.error('Selecione o cliente'); return; }
    ...
    try {
      ...
      const res = await apiFetch('/api/sales/quotations', {
        method: 'POST',
